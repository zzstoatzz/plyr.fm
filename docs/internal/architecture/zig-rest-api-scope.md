...
| `GET /v1/albums?artist_did={did}` | covered | canonical list-record albums for one artist with strict keyset pagination | album detail/membership, verified ingestion, global discovery, writes, viewer state |
...
The product coverage count is therefore **four read capabilities**: anonymous
track discovery, track detail, artist detail, and artist album discovery. The
...
artist lookups have one covered successor, one album listing has partial
coverage, and the remaining 214 have no implemented Zig mapping yet.
...
| `/albums` | 9 | 9 | canonical artist album collection partial; detail, membership, and mutations open |
...
| albums | `/albums/*` | ATProto list/track records, Postgres, R2 images | canonical artist collection started; add verified membership/detail and eliminate local-first finalization semantics |
