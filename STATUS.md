...
#### client-side writes, phase 0: the browser holds its own atproto session (#1948–#1950, August 31 — prod `2026.0901.065150`)

**why**: the backend writes every record on users' behalf, and that is a
design smell the client-writes plan
(`docs/plans/2026-08-31-client-side-writes.md`) now reckons with: records in
`fm.plyr.*` should be authored by the signed-in user's own browser session —
the standard atproto client pattern (rally, doodl, bsky's own app) — with
plyr's backend doing what an appview does: verify, index, mirror, serve.
the PDS accepts `createRecord` only with the user's own authorization, so
the OAuth session must live in the browser.

**what shipped**: the frontend is now also a public OAuth client
(`/oauth-client-metadata.json` served per origin from the same builder the
running client uses; session in IndexedDB via `@atproto/oauth-client-browser`,
lazily imported). sign-in chains the session once — first ever trip shows
the PDS consent screen, after that the round-trip is silent, and once
stored there is no navigation at all. `POST /ingest/record` is the verified
read-after-write bridge: the client names an `at://` URI in its own repo,
and the appview fetches the record from the caller's PDS and dispatches it
to the same ingest functions jetstream uses — present means create, absent
means delete, and the claim is never trusted. no feature flag: a browser
without a session falls back to the server endpoints. scope is only what
phase 1 needs (`repo:fm.plyr.like`, create+delete); later phases add scopes
via re-authorization, which prompts only for the delta.

**technical notes**: two follow-up fixes were the real lesson — CF Pages
re-bundles the worker with wrangler 3.x, which cannot parse
`@atproto/api`'s `import … with { type: 'json' }`, and whether that file
reaches the output was environment-dependent (local vite inlined the
package; CF's left it external). deployments show **Active** with a failed
Functions step, silently serving static assets with no worker.
`ssr.noExternal` + aliasing `tlds` to its JSON makes the output
deterministic. prod smoke: chained consent, echo create/delete round-trip
into the liked list, forged foreign-repo claim 403, silent re-login.
**next**: phase 1 — likes written by the browser, with parity gates on
time-to-indexed and error rate before the server path is considered
redundant.
...
**records are moving into the client's hands** (plan `docs/plans/2026-08-31-client-side-writes.md`; phase 0 #1948–#1950, prod `2026.0901.065150`): the browser now holds the user's own atproto OAuth session alongside the cookie, and `POST /ingest/record` gives client-authored records verified read-after-write. the direction: the file an artist uploads goes in their PDS as-is, plyr indexes/mirrors/serves, and the backend stops authoring records on anyone's behalf. **next**: phase 1 migrates likes to browser writes behind parity gates (p95 time-to-indexed, error rate); the resumable-upload transport (#1947) is the substrate the upload phase will reuse. the one engineering constraint to carry: `@atproto/api` must stay bundled (`ssr.noExternal`) — CF Pages' wrangler cannot parse its import-attributes syntax.
