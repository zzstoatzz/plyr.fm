...
### known issues
- **negation recovery is bounded by the projection sync, not the label stream**: #1695 made a *newly emitted* label take effect in ~0.8s, but clearing one still waits on `sync_operator_labels` (~5 min, measured 301s) because the SQL-side discovery filter reads `tracks.operator_labels`. Fail-closed direction, so not urgent — a track stays hidden slightly too long after a moderator clears it.
