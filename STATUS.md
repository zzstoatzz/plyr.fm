...
- **keyword search ranks trigram fuzz above lexical intent** ([#1523](https://github.com/zzstoatzz/plyr.fm/issues/1523), mechanism confirmed August 8): relevance is `similarity(title, query)` alone, which divides shared trigrams by the *union* of both strings — a long title is structurally punished, so "you don't kn" ranks its exact-prefix title 4th behind short fuzzy matches. The `ilike` substring check only qualifies rows, contributing nothing to rank; apostrophe variants (`'` vs `’`) fracture matching further. Fix in progress.
