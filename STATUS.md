...
**moderation: labels that act** (#1697, July 25): the arc of the window. `copyright-violation` now de-lists a track from radio and discovery instead of being inert, closing the "act on it" half of the 2026-01-02 legal review that #703 deferred and nobody returned to; adult labels stopped gating audio bytes, so a creator's permalink works for signed-out listeners again. The organizing idea is that **a label is a portable assertion and enforcement is local hosting policy** — adult defers to the listener, copyright cannot, because we serve the bytes. **next in this arc**: a moderation event log to carry per-track overrides, audit trail, and transparency; then triage of the three flagged-but-unlabeled tracks; then moderation UX before any automation.
...
### known issues
- **no per-track moderation override**: the only lever against a false-positive label is negating it, which conflates "this assertion is wrong" with "this assertion is right and I am allowing it anyway". Those are different statements and the second has nowhere to live. Needs a moderation event log (subject, actor, action, reason, timestamp) — the same table would carry the audit trail, transparency posts, and the reviewable proposed-action object any future automation would operate on. More urgent since #1697, because labels now have consequences.
- **three flagged tracks are unlabeled and untriaged**: 64 (user report #5), 1098, 1173 carry `copyright_scans.is_flagged` with no label, so #1697's de-listing does not apply to them. They need human triage — a fingerprint match is not a finding.
