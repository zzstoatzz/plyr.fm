...
### known issues
- copyright flags are silently wiped within ~5 min ([#1602](https://github.com/zzstoatzz/plyr.fm/issues/1602), open) — `sync_copyright_resolutions` assumes every flag emitted a label and flips `is_flagged=false` for URIs that were never labelled (broken since #703). the self-match suppression (#1341) is unrelated and correct; this is the separate, still-unfixed sync bug.
