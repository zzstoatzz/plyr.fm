...
#### choosing a file starts the upload, and the form shows what it knows about it (#1954, September 1 — prod `2026.0901.203801`)
...
and a `StagedTransfer` opens the resumable session from #1947 and sends the
parts immediately, with a progress bar and a `retry` that resumes from the
parts the server already holds. submit hands the transfer to the uploader,
which waits for it and then calls `finish` with the form; publishing, and
the PDS write behind it, still happen only there. re-choosing a file or
leaving the page aborts an unsubmitted transfer (the 24 h reaper cleans up
the session, as with a closed tab). `/record` now renders its recording
...
- a session-expired failure during the background transfer shows inline
  with retry and never redirects; submit's preflight still owns the
  draft-stash-and-bounce. an abort endpoint was considered and declined:
...
- **`just release` dies at `git checkout production-fe`** (September 1, twice): with no local `production-fe` and the branch on both the github and tangled remotes, git refuses the ambiguous checkout and the script exits after the GitHub release is created and the backend deploy has started. finish by hand — `git push origin main:production-fe` and `git push tangled main --tags` — and do not re-run `just release`, the tag already exists. the fix is `git checkout --track origin/production-fe` (or pushing `main:production-fe` directly) in `scripts/release`.
