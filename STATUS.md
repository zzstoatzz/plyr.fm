...
### known issues
- **the production `requirepass` cutover has not been run** ([#1782](https://github.com/zzstoatzz/plyr.fm/issues/1782)): staging is done and verified; production still accepts unauthenticated connections from anything on the org's private network. The config is merged (#1786), so what remains is a `fly deploy` plus a coordinated `DOCKET_URL` rotation on `relay-api`. Expect ~25s of docket disruption — now survivable rather than an outage (#1787), but not zero. Held for sign-off.
