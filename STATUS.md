...
### December 2025 (Dec 3-8)

#### lexicons overview documentation (PR #514, Dec 8)

**what shipped**:
- comprehensive lexicon documentation at `docs/lexicons/overview.md`
- explains the four plyr.fm lexicons: `fm.plyr.track`, `fm.plyr.album`, `fm.plyr.list`, `fm.plyr.actor.profile`
- documents schema structure, ATProto conventions, and usage patterns
- includes examples for each lexicon type
- covers blob references, strongRefs, and record relationships

---

#### detail page button layouts and AddToMenu positioning (PR #515, Dec 8)

**what shipped**:
- fixed button layout on track/album detail pages - buttons now stack properly on mobile
- fixed AddToMenu positioning - menu now anchors correctly relative to parent button
- improved z-index handling to prevent menu clipping
...
- **PR #509**: playlist menus and link previews - fixed stopPropagation blocking links
...
- clicking "create new playlist" previously navigated to `/library?create=playlist`
...
**playlists** (full CRUD):
- full CRUD operations: create, rename, delete, reorder tracks, upload cover art
- playlist detail page with drag-and-drop reordering (desktop) and touch reordering (mobile)
- "add to playlist" menu on tracks throughout the app
...
- automatic sync of albums, liked tracks, and profile on login (fire-and-forget background tasks)
- playlists synced on create/modify (not at login) to avoid N playlist syncs on every login
...
**sync architecture**:
- profile, albums, liked tracks: synced on login via background tasks
- playlists: synced on create/modify only
- sync tasks don't block responses (~300-500ms endpoint, PDS calls happen in background)
...
**problem**: artwork uploads from iOS Photos library silently failed.

**root cause**: iOS stores photos in HEIC format. when selected, iOS converts to JPEG but keeps `.heic` filename. backend validated format using only filename extension.

**fix**: backend now prefers MIME `content_type` over filename extension for format detection.
...
- `show_sensitive_artwork` user preference (default: hidden)
...
- SSR-safe filtering: link previews exclude sensitive images
...
**what shipped**:
- native teal.fm scrobbling: plays recorded to user's PDS using teal's ATProto lexicons
- scrobble triggers at 30% or 30 seconds (whichever comes first)
- user preference toggleable from settings
- lexicons used: `fm.teal.alpha.feed.play` (scrobbles), `fm.teal.alpha.actor.status` (now-playing)
...
- keyboard navigation, lazy loading, glassmorphism modal styling
...
### December 2025 (Dec 1-2)

See `.status_history/2025-12.md` for detailed work including:
- light/dark theme system (PR #441)
- tag filtering system (PRs #431-434)
- queue touch reordering (PR #428)
- connection pool resilience for Neon cold starts
- now-playing API (PR #416)
- admin UI improvements for moderation (PRs #408-414)

### November 2025

See `.status_history/2025-11.md` for detailed November development history including:
- ATProto labeler and copyright moderation (PRs #382-395)
...
- iOS PWA audio may hang on first play after backgrounding - service worker caching interacts poorly with 307 redirects. PR #466 added `NetworkOnly` for audio routes. workaround: delete home screen bookmark and re-add.
