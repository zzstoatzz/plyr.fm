...
#### uploads became resumable sessions, and "slow" stopped meaning "dead" (#1947, August 29 — prod `2026.0901.065150`)

**why**: woody's 391–579s uploads (August 24–25) exposed the transport, not
just the toast: `POST /tracks/` held one request open for the whole transfer
and then staged the bytes to R2 *inside the request* — the 579s trace was
346s of body transfer followed by 231s of `R2 save` with the browser parked
at 100%. the client's fixed 300s `xhr.timeout` (which counts the whole
request) called those uploads failed; one was re-uploaded 15 minutes later.
bumping the timeout was rejected as glossing over the design; the shape that
shipped is the one bluesky's video service and stream.place converged on —
many short requests, each independently timed out and retried, with the
server acknowledging bytes as it gets them.

**what shipped**: `POST /tracks/uploads` opens an R2 multipart upload on a
staged key in the private bucket; each 10 MiB part is its own `PUT` (3 in
flight, a 60s *stall* timeout measured from the last progress event, 5
attempts with backoff); `finish` validates the same metadata as
`POST /tracks/` via the shared `parse_upload_metadata`, checks the part
list, and enqueues the worker. the worker's new phase 0 settles the staged
bytes — stream once to disk while hashing, scan duration/ALAC, server-side
copy to `audio/<hash>.<ext>` (or `uploadBlob` for private media) — and the
rest of the pipeline runs unchanged. `POST /tracks/` remains for the SDK;
`reap_abandoned_transfers` closes sessions idle 24h. docs:
`docs/internal/backend/resumable-uploads.md`.

**technical notes**: verified locally against real R2 dev buckets and the
zat PDS before the PR (a scoped R2 token minted and deleted for the run),
then on staging and prod through the real browser UI — a 32 MB upload
transfers in ~10s and publishes at ~24s, byte-exact on `audio.plyr.fm`.
the review pass caught a real one: FastAPI hands starlette's `UploadFile`
where the helper hinted fastapi's subclass, which beartype rejects — every
upload with cover art would have 500d. incidental: the test compose
postgres was pinned at `max_connections=100` while `-n auto` scales with
cores; `main` itself peaked at 94/100 on an 18-worker machine, so it is
300 now.

#### client-side writes, phase 0 — REVERTED (#1948–#1950 shipped August 31 in prod `2026.0901.065150`; reverted September 1)

**why it was reverted**: phase 0 registered the frontend as a second OAuth
client with its own `client_id`, and chained `signIn` right after the cookie
login. the authserver keys consent to `client_id`, so every user's next
sign-in showed two authorization screens: the backend's confidential client
and then the browser's public client — for a scope nothing used yet. that is
two login flows, and there must never be more than one. progressive scope
upgrade is fine when the user reaches for a feature that needs it; a second
authorization at sign-in is not.

**what was undone**: the browser OAuth client (`frontend/src/lib/atproto/`),
the `/oauth-client-metadata.json` and `/atproto/callback` routes, the
post-login chaining on `/portal`, `/library`, `/profile/setup`, the
`POST /ingest/record` echo endpoint, the `@atproto/api` SSR bundling
workaround, and the `@atproto/oauth-client-browser` dependency. the plan
(`docs/plans/2026-08-31-client-side-writes.md`) stays as the direction, but
its sign-in section is wrong and must be redesigned before any phase ships:
the browser must obtain repo-write capability inside the one existing login
(or lazily, on first use of a feature that needs it), never as a separate
flow.
