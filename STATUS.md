...
#### status maintenance and documentation (PRs #513-520, Dec 8)
- improved status maintenance prompt clarity (#520)
- made playlist detail page publicly viewable (#519)
- fixed unique branch names for status maintenance (#517)
- fixed detail page button layouts and AddToMenu positioning (#515)
- added lexicons overview documentation (#514)

#### playlist release fast-follow fixes (PRs #507-510, Dec 7)
- inline playlist creation to avoid navigation-based playback interruption (#510)
- playlist menus and link previews (#509)
- invalidate layout data after token exchange (#508)
- include image_url in playlist SSR data for og:image (#507)
- moved ATProto sync to login callback for better performance (#505)

**the navigation bug** (PR #510): clicking "create new playlist" previously navigated to `/library?create=playlist`, causing SvelteKit to reinitialize the layout and stop playback. fixed with inline create form that avoids navigation.

**full context**: see `.status_history/2025-12.md` for detailed playlist implementation notes

#### playlists and library hub (PR #499, Dec 6-7)
- full CRUD for playlists (create, rename, delete, reorder, cover art upload)
- playlist detail pages with drag-and-drop reordering
- "add to playlist" menu on tracks with inline create option
- unified `/library` page with tabs: liked, playlists, albums
- `fm.plyr.list` lexicon for syncing playlists/albums to user PDSes
- automatic sync of albums, liked tracks, and profile on login
- public liked pages for any user (`/liked/[handle]`)
- fixed 32 svelte-check warnings (ARIA roles, button nesting)

**sync architecture**: profile/albums/liked synced on login; playlists synced on create/modify to avoid N syncs per login. all sync happens fire-and-forget in background.

**related issues**: #221, #146, #498

#### settings consolidation (PR #496, Dec 6)
- separated `/settings` (preferences) from `/portal` (content & data)
- `/settings`: theme, accent color, auto-advance, sensitive artwork, timed comments, teal.fm, developer tokens
- `/portal`: profile, tracks, albums, export, delete account

#### polish and fixes (PRs #471-495, Dec 4-6)
- teal.fm scrobbling integration (#467) - records plays to user's PDS using teal's ATProto lexicons
- sensitive image moderation (#471-488) - `sensitive_images` table with `show_sensitive_artwork` preference
- mobile artwork upload fix (#489) - use content_type instead of filename extension for iOS HEIC uploads
- bufo easter egg improvements (#491-492) - configurable exclude/include patterns via env vars
- keyboard shortcuts for playback (#474)
- cloudflare analytics dashboard script (#473)
- prevent duplicate teal.fm scrobbles from race condition (#472)

#### unified search (PR #447, Dec 3)
- `Cmd+K` / `Ctrl+K` opens search modal from anywhere
- fuzzy matching across tracks, artists, albums, tags using PostgreSQL `pg_trgm`
- keyboard navigation, glassmorphism styling
- follow-up polish: mobile search icon, theme-aware styling, ILIKE fallback (#449-463)

#### theme system and mobile UX (PRs #441-445, Dec 2-3)
- light/dark theme with CSS custom properties, system preference support
- `ProfileMenu` component with touch-optimized 44px tap targets
- dedicated `/upload` page extracted from portal
- player scroll timing improvements
- CI optimization: path-based pre-commit skips (~10s instead of ~1m20s)

#### tag filtering and SDK support (PRs #431-438, Dec 2)
- users can hide tracks by tag via eye icon filter in discovery feed
- default hidden tags: `["ai"]` for new users
- tag detail pages at `/tag/[name]`
- SDK tag support in plyr-python-client v0.0.1-alpha.10
- bufo easter egg for tracks tagged with `bufo` (semantic toad GIF matching)

#### infrastructure improvements (Dec 2)
- queue touch reordering for mobile (#428)
- connection pool resilience for Neon cold starts (pool_size: 5→10, max_overflow: 0→5)
- fixed 5-minute API outage caused by cold start pool exhaustion

#### November-December 1 features
- now-playing API endpoints (#416)
- admin UI improvements for moderation (#408-414)
- ATProto labeler and copyright moderation (#382-395)
- developer tokens with independent OAuth grants (#367)
- platform stats and media session integration (#359-379)
- export & upload reliability improvements (#337-344)
