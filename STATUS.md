...
### known issues
- **`plyr-redis` requires no authentication** ([#1782](https://github.com/zzstoatzz/plyr.fm/issues/1782)): no `--requirepass`, no secrets set on the app, connection string is a bare `redis://plyr-redis.internal:6379`. It is not internet-exposed — `fly ips list` returns nothing — so the exposure is "any workload on the org's private network" rather than "the internet". The credentials it held are now encrypted (#1783), but it still carries docket state, rate-limit counters, and the discovery cache, and it remains the payoff step that makes any other foothold worth more. The fix is a coordinated password + `DOCKET_URL` rotation across `relay-api`; there is an unavoidable window where one side has the password and the other does not, so it wants doing deliberately rather than opportunistically.
- **a `did:web` PDS endpoint is unvalidated, and PDS-hosted audio is scanned from a mutable source** ([#1778](https://github.com/zzstoatzz/plyr.fm/issues/1778)): `artist.pds_url` is written straight from the resolved miniDoc with no `is_safe_url` check, and `resolve_audio_url` interpolates it into a blob URL handed to AudD, Modal, and Replicate — all three fetch it themselves. The backend never fetches it, so this is SSRF *by proxy* against third parties rather than a route into our network. The more interesting half: for a `did:web` track those bytes are served by the user's own host on every request, so a clean copyright scan does not mean the track is clean. Pinning the scan to `pds_blob_cid` means the backend starts fetching and hashing blobs on the track-creation hook — the path #1519 deliberately made non-blocking — so it is a real change, not a validation tweak.
...
- **undiagnosed `SELECT neondb` errors on staging** (August 8): five error-level records against `SELECT ... FROM jobs` in the minute after a staging deploy, and 29 in the same shape on August 7. Staging-only, never production, no exception attached. The convenient explanation is restart noise, which is exactly why it has not been accepted — nobody has actually looked.
...
- **the write-echo alert is live but unexercised** (#1775): no `pds record write` has occurred in production since the release, so the blackout alert's write side has never been observed non-zero. The unit tests pin the emit contract and the alert fails quiet (no writes → no fire), so the worst case is silence, not a false page — but the first real user write is the actual verification.
