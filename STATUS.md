...
### known issues
- **the upload XHR gives up at 300s while real uploads run longer** (observed August 27): `xhr.timeout` in `uploader.svelte.ts` is 300s, but woody's successful August 24 upload held the `POST /tracks/` open for 578s — so a slow transfer toasts a timeout error for an upload that then succeeds server-side. either the timeout should scale with file size or the copy should admit the upload may still land. not yet filed as an issue.
