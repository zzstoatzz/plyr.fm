...
#### 500 followers, and what the numbers actually say (August 5)

**why**: the @plyr.fm bluesky account crossed 500 followers with very little
self-promotion, which prompted the first full usage accounting since launch.
Recorded here both for the numbers and for what measuring them revealed about
what we can and cannot know.

**the numbers** (as of August 5, 2026):
- lifetime (prod DB + `GET /stats`): 999 tracks (121 hours of audio) from 97
  publishing artists; 442 atproto accounts have signed in, accruing ~40–50/month
  with no dead months; 13,384 plays; 474 likes; 141 albums; 44 playlists; 48 jams.
  December's 432-track month was one artist (pyxorium.com) uploading a 379-track
  back catalog — verified in the DB before annotating, because the first guess
  ("the continuous publisher") was wrong.
- last 30 days (cloudflare edge + logfire): 564k edge requests, 184 GB served,
  ~74% of bytes from cache; peak 1,599 unique visitors/day (avg ~899) against
  90 distinct signed-in users — the ~100× gap is structural, most listening is
  logged-out. 109 5xx in 30 days (~0.05%). One 96k-request edge burst (Jul 19)
  and one 4,278-threat bot swarm from a single NL source (Aug 4) were both fully
  absorbed at the edge; the app never saw either.

**what measuring revealed**:
- **plays-over-time is unknowable historically.** `play_count` is a counter on
  tracks, not timestamped events, so the only trend window is logfire's 14-day
  retention on `/tracks/{track_id}/play`: ~73 plays/day, flat. That is above the
  ~48/day lifetime average, but the comparison is confounded — radio plays only
  began counting on July 1 (#1622). If future recaps want a real curve, we have
  to start recording one: an append-only play-events table, or a daily snapshot
  of `/stats` totals whose deltas become the curve. Deliberately not built yet —
  new surface, not yet decided.
- **the honest growth story is accumulation, not acceleration**: the catalog and
  account base rise steadily; daily listening is a steady pulse (~8 signed-in
  users/day, ~700–900 visitors/day, slightly below the early-July highs).

#### /atlas — a 2D semantic map of the catalog (#1766, #1767, #1768, august 4-5)

**why**: discovery has been all lists (feeds, tags, radio), but we already store a 512-dim CLAP embedding for every track in turbopuffer for mood search — exactly the input a spatial map needs. `/atlas` (unlisted, noindex) puts every public track on a pan/zoom canvas positioned by how it *sounds*, adapted from pub-search's atlas pipeline and renderer.

**what shipped**:
- `scripts/build_atlas.py`: standalone uv script — radio-corpus eligibility rules (public/supporters, ungated, active artist, no adult-audio/copyright labels), turbopuffer vector export, PCA→50 then UMAP→2D for display plus a separate UMAP→10D for HDBSCAN clustering, two cluster tiers (~17 coarse regions / ~56 fine), c-TF-IDF keywords refined into 2-4 word region names by claude haiku
- daily freshness: `.github/workflows/build-atlas.yml` rebuilds and uploads `atlas.json` to the stats bucket; new `GET /stats/atlas` proxies it like `/stats/costs` (15min cache) — the pub-search lesson is that a static map fossilizes
- canvas 2D renderer: sprite-stamped points over a spatial grid index, gaussian nebulae per cluster, one shared label-collision economy (regions → fine clusters → track titles by plays+likes → artist handles); semantic zoom ends at actual cover art (96px webp, drawn flat — no effects on artwork)
- click a track → normal footer player; artists with 2+ tracks render as avatar circles at their catalog centroid → profile. theme-aware, touch (drag/pinch), fits the viewport like `/radio`
- #1767: first CI dispatch failed because `NEON_DATABASE_URL_PRD` is in sqlalchemy form (`postgresql+asyncpg://`) and psycopg rejects it — normalize the same way `export_costs.py` does
- #1768 (from review on prod): larger dot/star sprites, artist circles visible from the whole-map view with a zoom-scaled size cap so a dominant catalog can't own the overview, artist labels join the economy at zoom 2 instead of 3.5

**technical notes**:
- clustering runs on the 10D projection, not the 2D display — clustering the display projection turns its artifacts into cluster boundaries (measured in pub-search: fine tier scored worse than chance in 2D). PCA is float64 because randomized SVD overflows in float32 at these magnitudes
- outliers snap to the nearest centroid for display but are excluded from label evidence; labels come from core members only
- deliberately deferred: nav link, live radio overlay, atlas-as-PDS-blob, deep links, on-map search. the WebGL rotating-planet layer from pub-search was not ported — covers want to be shown flat

#### codify that public audio is analyzed and derived data is published (#1769, 2026-08-04)

**why**: the atlas (#1766) crossed a line the legal pages hadn't caught up to. before, everyone whose tracks sat in the vector store had accepted the Feb 2026 terms (embedding pipeline and version-aware re-acceptance shipped together in #848); the atlas now publishes embedding-derived data about the entire public catalog to anonymous visitors with no per-artist ask. this codifies the new stance instead of leaving the old one implied.

**what shipped**:
- terms §2 (Content): public content is analyzed automatically (embeddings, genre classification, copyright fingerprinting, aggregate catalog views) and derived data (similarity coordinates, suggested genres, cluster labels) may be publicly visible
- privacy §3: the same statement from the data-use side — processors (Modal, turbopuffer, Replicate, AudD, Anthropic) were already disclosed in §4
- `terms_last_updated` → 2026-08-04, so the version-aware acceptance flow re-prompts existing users; consent to the new state is collected the same way as for #848, not assumed
- glossary: new `atlas` entry; artists.md "indexed for discovery" now names the surfaces (search, radio, catalog map)
- pre-commit: `check-legal-dates` invokes `python3` (bare `python` doesn't exist on stock macOS, so the hook failed before it could run)

**technical notes**: deliberately unchanged — the `vibe-search` flag (UI gating, not consent), auto-tag stays opt-in per upload (mutation consent for writing tags to your track), and the jetstream known-DID ingest gate (opening ingest beyond signed-in artists is a separate product decision).

#### embeds survive sandboxed iframes (#1770, August 4)

**why**: a plyr.fm radio embed nested inside a sandboxed iframe (e.g. a Leaflet document) never hydrated — it sat frozen on its SSR'd "tuning…" state. an iframe sandboxed without `allow-same-origin` gets an opaque origin, and merely *accessing* `window.localStorage`/`sessionStorage` there throws a `SecurityError` — `typeof localStorage !== 'undefined'` and `browser` guards don't catch it. `jam.svelte.ts` touched `sessionStorage` in a module-level singleton constructor, so the whole bundle threw at import time and hydration never ran.

**what shipped**:
- `frontend/src/lib/utils/safe-storage.ts` — `safeLocalStorage`/`safeSessionStorage` wrappers with every access (including the property access itself) in try/catch; the MDN-documented idiom for this problem
...
- backend CORS now accepts the literal `Origin: null` that sandboxed iframes send — without it the embed would hydrate but every fetch would be blocked
- `safe-storage.test.ts` covers the throwing-storage path plus normal round-trips; CORS tests extended for `null` in all three environments

**technical notes**: allowing `Origin: null` loosens nothing — prod/staging already allow any https origin, and session cookies are HttpOnly and scoped to plyr.fm. no in-memory persistence fallback for sandboxed contexts: embeds just don't persist preferences, which is correct for them. adopting something like `svelte-persisted-store` was deliberately out of scope — it would mean restructuring every `$state` manager. verified by rendering the embed in a `sandbox="allow-scripts"` iframe against a local wrangler pages build.
...
**verified against a real boundary** (staging, 08:00 UTC, August 4): the in-flight
track ("Intro Recording", due to end 5s past the boundary) played through the boundary
and ended naturally at 08:00:05; the new rotation started at track 0 at that exact
instant (observed at 08:00:20 as its head, "burnout", at progress 15s = 20 − 5), and
30s sampling for the following 12 minutes recorded zero mid-song switches. one
monitoring lesson recorded alongside: the watcher initially flagged this as a
violation by comparing a 3-minute-old "remaining time" against a sample taken after
the track had legitimately finished — checks on a shared clock have to re-derive
what *should* be playing at the later instant, not diff two stale snapshots.
