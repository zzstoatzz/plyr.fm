...
**unified feedback system** with two modes: bug reports (→ GitHub issues) and content reports (→ moderation service).

**content reports (PR #805)**:
- `user_reports` table in moderation service with full CRUD
- `POST /moderation/reports` gateway endpoint with 10/hour rate limiting
- entity search, reason selection (copyright, abuse, spam, explicit, other), optional description
- FeedbackModal component with two-mode UI

**moderation admin UI (PRs #806-807)**:
- tab navigation: copyright flags, user reports, sensitive images
- report cards show target type/id, reason, status, reporter handle
- resolution flow with actions: resolve, dismiss, investigating

**UX polish (PRs #808-812)**:
- report cards link to target entities and reporter profiles
- DM notification sent to admin on new reports
- flag icon (red for content reports) replaces chat bubble
