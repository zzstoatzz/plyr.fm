...
pub const cursor_prefix = "plscur_";

const Options = struct {
    limit: usize = default_limit,
    cursor: ?[]const u8 = null,
    owner_did: ?[]const u8 = null,
};
...
pub fn execute(
    ...
    if (options.owner_did) |owner_did| {
        if (zat.Did.parse(owner_did) == null) return .invalid_request;
    }
    const scope = if (options.owner_did) |owner_did|
        std.fmt.allocPrint(allocator, "owner:{s}", .{owner_did}) catch return .unavailable
    else
        "global";
    ...
    const after: ?scoped_record_cursor.Cursor = if (options.cursor) |token| blk: {
        ...
        if (!std.mem.eql(u8, parsed.collection() orelse return .invalid_request, list_collection))
            return .invalid_request;
        if (options.owner_did) |owner_did| {
            if (!std.mem.eql(u8, parsed.authority(), owner_did)) return .invalid_request;
        }
        break :blk decoded;
    } else null;
...
fn parseOptions(allocator: std.mem.Allocator, target: []const u8) !Options {
    ...
    while (try pairs.next()) |pair| {
        ...
        } else if (std.mem.eql(u8, pair.name, "owner_did")) {
            if (saw_owner or pair.value.len == 0) return error.InvalidQuery;
            saw_owner = true;
            result.owner_did = pair.value;
...
test "playlist collection permits global discovery and validates an owner scope" {
    const Fake = struct {
        ...
        fn list(
            ...
            if (request.kind != .playlist) return error.CorruptProjection;
            if (request.owner_did) |owner| {
                if (!std.mem.eql(u8, owner, "did:plc:owner")) return error.CorruptProjection;
            }
            return &.{};
        }
        fn get(
    ...
    try std.testing.expectEqual(@as(usize, 0), global.found.data.len);
    const owner = execute(
        arena.allocator(),
        fake.store(),
        "fm.plyr.dev.list",
        "fm.plyr.dev.actor.profile",
        "/v1/playlists?owner_did=did%3Aplc%3Aowner",
    );
    try std.testing.expectEqual(@as(usize, 0), owner.found.data.len);
